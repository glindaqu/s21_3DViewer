...
void apply_projection_type_setting(ViewerAppWindow *self) {
  const gchar *projection_type =
      g_settings_get_string(self->settings, "projection-type");
...
void apply_edge_thickness_setting(ViewerAppWindow *self) {
  set_edge_thickness(self,
                     g_settings_get_int(self->settings, "edge-thickness"));
}

void apply_edge_color_setting(ViewerAppWindow *self) {
  GVariant *color_variant = g_settings_get_value(self->settings, "edge-color");
  g_variant_get(color_variant, "(iiii)", &self->edge_color[0],
                &self->edge_color[1], &self->edge_color[2], NULL);
  g_variant_unref(color_variant);
}

void apply_background_color_setting(ViewerAppWindow *self) {
  GVariant *color_variant =
      g_settings_get_value(self->settings, "background-color");
  g_variant_get(color_variant, "(iiii)", &self->background_color.red,
...
void apply_point_color_setting(ViewerAppWindow *self) {
  ...
  g_variant_get(color_variant, "(iiii)", &self->point_color[0],
                &self->point_color[1], &self->point_color[2], NULL);
  g_variant_unref(color_variant);
}
