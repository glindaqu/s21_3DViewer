...
static void save_image_as_bmp(ViewerAppWindow *self, const char *filename) {
  ...
  if (pixbuf) {
    ...
    for (int y = 0; y < height; y++) {
      memcpy(flipped_pixels + y * rowstride,
...
static void save_bmp_callback(GtkFileDialog *dialog, GAsyncResult *result,
                              gpointer user_data) {
...
static void on_save_bmp_activate(GSimpleAction *action, GVariant *parameter,
  ...
  gtk_file_dialog_save(dialog, GTK_WINDOW(self), NULL,
                       (GAsyncReadyCallback)save_bmp_callback, self);
}
...
static void save_jpeg_callback(GtkFileDialog *dialog, GAsyncResult *result,
                               gpointer user_data) {
  ViewerAppWindow *self = VIEWER_APP_WINDOW(user_data);
  GFile *file = gtk_file_dialog_save_finish(dialog, result, NULL);
  if (file) {
...
static void on_save_jpeg_activate(GSimpleAction *action, GVariant *parameter,
  ...
  gtk_file_dialog_save(dialog, GTK_WINDOW(self), NULL,
                       (GAsyncReadyCallback)save_jpeg_callback, self);
}

void viewer_app_window_add_save_actions(ViewerAppWindow *self) {
  g_autoptr(GSimpleAction) save_bmp_action =
      g_simple_action_new("save_bmp", NULL);
  g_signal_connect(save_bmp_action, "activate",
                   G_CALLBACK(on_save_bmp_activate), self);
  g_action_map_add_action(G_ACTION_MAP(self), G_ACTION(save_bmp_action));

  g_autoptr(GSimpleAction) save_jpeg_action =
      g_simple_action_new("save_jpeg", NULL);
  g_signal_connect(save_jpeg_action, "activate",
                   G_CALLBACK(on_save_jpeg_activate), self);
